...
## Demo Overview
In this demo, we will upload sample tunnel log traffic from Zeek IDS to Splunk SIEM and perform various analyses to gain insights into tunneling activities within the network.
...
```
^(?<timestamp>\d+\.\d+)\t(?<session_id>\w+)\t(?<src_ip>\d+\.\d+\.\d+\.\d+)\t(?<src_port>\d+)\t(?<dest_ip>\d+\.\d+\.\d+\.\d+)\t(?<dest_port>\d+)\t(?<tunnel_type>[^\t]+)\t(?<tunnel_action>[^\t]+)

```
![splunk_searsh](screenshots/tunnel1.png)
